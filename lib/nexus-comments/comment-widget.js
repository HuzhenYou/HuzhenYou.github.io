...
(function (global) {
  ...
  var DEFAULT_LABELS = {
    ...
    hoursAgo: '小时前',
    daysAgo: '天前',
    /* 实名评论（options.auth 开启时才用到） */
    authHint: '评论需手机号验证（仅用于实名合规，不会公开）',
    authLogin: '登录 / 注册',
    authLoggedAs: '当前身份：',
    authLogout: '退出',
    authTitle: '手机号验证后即可评论',
    authDesc:
      '依据网络安全相关规定，发表评论前需完成手机号验证。手机号仅用于实名核验，不会公开显示；验证一次后一年内无需重复验证。',
    authPhone: '手机号',
    authCode: '短信验证码',
    authNick: '昵称（首次验证时填写，公开显示）',
    authSendCode: '获取验证码',
    authResendIn: '秒后可重发',
    authSubmit: '验证并发表评论',
    authSubmitOnly: '完成验证',
    authCodeSent: '验证码已发送，请查收短信',
    errPhoneInvalid: '请输入正确的手机号',
    errCodeInvalid: '请输入 6 位验证码'
  };
  ...
  function CommentWidget(options) {
    ...
    // 实名评论（可选）：auth: true | { apiBase }。开启后浏览零门槛，只在「提交评论」
    // 那一刻要求手机号验证；验证后的会话存 localStorage，服务端默认一年免验。
    var authOpt = opts.auth;
    this.auth = null;
    if (authOpt === true || (authOpt && typeof authOpt === 'object')) {
      this.auth = assign({ apiBase: this.apiBase + '/auth' }, authOpt === true ? {} : authOpt);
      this.auth.apiBase = String(this.auth.apiBase).replace(/\/+$/, '');
    }
    this.token = this.auth ? lsGet('auth_token') : '';
    this.user = null;
    if (this.auth && this.token) {
      try {
        this.user = JSON.parse(lsGet('auth_user') || 'null');
      } catch (e) {
        this.user = null;
      }
    }
    /** 被登录门槛拦下的那次提交，验证通过后自动续提 */
    ...
    if (this.auth) this._refreshAuth();
  }
  ...
  CommentWidget.prototype._build = function () {
    ...
    // 实名状态条（未登录：提示 + 登录入口；已登录：昵称 + 退出）
    if (this.auth) {
      this.authBar = el('div', this._c('authBar', 'nxc-auth-bar'));
      root.appendChild(this.authBar);
      this._renderAuthBar();
    }
  ...
  CommentWidget.prototype._buildForm = function (parentId, onSuccess, onCancel) {
    ...
    emailInput.placeholder = this.requireEmail ? L.emailRequired : L.email;
    ...
    if (!this.auth) wrap.appendChild(metaRow);
  ...
  CommentWidget.prototype._submit = function (form, parentId, onSuccess) {
    ...
    if (this.auth && !this.token) {
      this.pendingSubmit = { form: form, parentId: parentId, onSuccess: onSuccess };
      this._openAuthModal();
      return;
    }
    ...
    if (this.auth && this.token) headers['Authorization'] = 'Bearer ' + this.token;
    ...
    fetch(this.apiBase, {
      ...
      .then(function (r) {
        if (self.destroyed) return;
        var json = r.json;
        if (r.status === 401 && self.auth && json && json.needs_login) {
          // 会话过期/被吊销：清本地态，弹验证框，通过后自动续提这条评论
          self._clearAuth();
          self.pendingSubmit = { form: form, parentId: parentId, onSuccess: onSuccess };
          self._openAuthModal();
          return;
        }
        if (r.status === 429) {
        ...
        if (json.rejected) {
          // 未通过审核：提示按错误态显示，保留输入（可能误判，方便改后重提）
          self._showMsg(form, json.message || L.rejected, true);
          return;
        }
        form.textarea.value = '';
        if (json.pending) {
  ...
  CommentWidget.prototype._authFetch = function (path, method, body) {
    var headers = { 'Content-Type': 'application/json' };
  ...
  CommentWidget.prototype._setAuth = function (token, user) {
    this.token = token || '';
    this.user = user || null;
    lsSet('auth_token', this.token);
    lsSet('auth_user', this.user ? JSON.stringify(this.user) : '');
    this._renderAuthBar();
  };

  CommentWidget.prototype._clearAuth = function () {
    this._setAuth('', null);
  };

  /** 页面加载时校验本地会话是否仍有效（过期 / 被吊销则清掉） */
  CommentWidget.prototype._refreshAuth = function () {
    var self = this;
    ...
    this._authFetch('/me')
      .then(function (r) {
        ...
        if (r.json && r.json.success && r.json.user) self._setAuth(self.token, r.json.user);
      })
      .catch(function () {
        /* 网络异常先信本地缓存；提交时服务端还会再验一次 */
      });
  };

  CommentWidget.prototype._renderAuthBar = function () {
    var self = this;
    var L = this.labels;
    var bar = this.authBar;
    if (!bar) return;
    while (bar.firstChild) bar.removeChild(bar.firstChild);
    if (this.user) {
      var left = el('span', '', L.authLoggedAs);
      left.appendChild(el('span', 'nxc-auth-user', this.user.nickname || ''));
      bar.appendChild(left);
      var out = el('button', 'nxc-link-btn', L.authLogout);
      out.type = 'button';
      this._on(out, 'click', function () {
        self.logout();
      });
      bar.appendChild(out);
    } else {
      bar.appendChild(el('span', '', L.authHint));
      var login = el('button', 'nxc-link-btn', L.authLogin);
      login.type = 'button';
      this._on(login, 'click', function () {
        self._openAuthModal();
      });
  ...
  /** 退出：吊销服务端会话 + 清本地 */
  CommentWidget.prototype.logout = function () {
    var self = this;
    if (!this.auth) return;
    var p = this.token ? this._authFetch('/logout', 'POST', {}) : Promise.resolve();
  ...
  CommentWidget.prototype._openAuthModal = function () {
    var self = this;
    var L = this.labels;
    if (this.modal) return;

    var mask = el('div', this._c('modal', 'nxc-modal-mask'));
    var box = el('div', 'nxc-modal');
    box.setAttribute('role', 'dialog');
    box.setAttribute('aria-modal', 'true');
    box.appendChild(el('div', 'nxc-modal-title', L.authTitle));
    box.appendChild(el('p', 'nxc-modal-desc', L.authDesc));

    var phone = el('input', this._c('input', 'nxc-input'));
    phone.type = 'tel';
    phone.placeholder = L.authPhone;
    phone.maxLength = 11;
    phone.autocomplete = 'tel';
    phone.inputMode = 'numeric';
    box.appendChild(phone);

    var codeRow = el('div', 'nxc-code-row');
    var code = el('input', this._c('input', 'nxc-input'));
    code.type = 'text';
    code.placeholder = L.authCode;
    code.maxLength = 6;
    code.autocomplete = 'one-time-code';
    code.inputMode = 'numeric';
    var sendBtn = el('button', 'nxc-code-btn', L.authSendCode);
    sendBtn.type = 'button';
    codeRow.appendChild(code);
    codeRow.appendChild(sendBtn);
    box.appendChild(codeRow);

    var nick = el('input', this._c('input', 'nxc-input'));
    nick.type = 'text';
    nick.placeholder = L.authNick;
    nick.maxLength = MAX_NICK;
    nick.autocomplete = 'nickname';
    nick.value = lsGet('nick');
    box.appendChild(nick);

    var actions = el('div', 'nxc-actions');
    var ok = el(
      'button',
      this._c('submit', 'nxc-submit'),
      this.pendingSubmit ? L.authSubmit : L.authSubmitOnly
    );
    ok.type = 'button';
    var cancel = el('button', this._c('cancel', 'nxc-cancel'), L.cancel);
    cancel.type = 'button';
    actions.appendChild(ok);
    actions.appendChild(cancel);
    box.appendChild(actions);

    var msg = el('div', 'nxc-msg');
    msg.style.display = 'none';
    box.appendChild(msg);

    mask.appendChild(box);
    this.root.appendChild(mask);
    this.modal = { mask: mask, phone: phone, code: code, nick: nick, sendBtn: sendBtn, ok: ok, msg: msg, busy: false };

    this._on(sendBtn, 'click', function () {
      self._sendCode();
    });
    this._on(ok, 'click', function () {
      self._verifyCode();
    });
    this._on(cancel, 'click', function () {
      self.pendingSubmit = null;
      self._closeAuthModal();
    });
    this._on(mask, 'click', function (e) {
      if (e.target === mask) {
        self.pendingSubmit = null;
        self._closeAuthModal();
      }
    });
    this._on(code, 'keydown', function (e) {
      if (e.key === 'Enter') self._verifyCode();
    });
    phone.focus();
  };

  CommentWidget.prototype._closeAuthModal = function () {
    if (this.codeTimer) {
  ...
  CommentWidget.prototype._modalMsg = function (text, isError) {
    var m = this.modal;
    if (!m) return;
    m.msg.textContent = text;
    m.msg.className = isError ? 'nxc-msg nxc-msg-error' : 'nxc-msg nxc-msg-notice';
    m.msg.style.display = '';
  };

  CommentWidget.prototype._sendCode = function () {
    var self = this;
    var L = this.labels;
    var m = this.modal;
    if (!m || m.sendBtn.disabled) return;
    var phone = m.phone.value.trim();
    if (!PHONE_RE.test(phone)) {
      this._modalMsg(L.errPhoneInvalid, true);
      m.phone.focus();
      return;
    }
    m.sendBtn.disabled = true;
    this._authFetch('/send-code', 'POST', { phone: phone })
      .then(function (r) {
        if (self.destroyed || self.modal !== m) return;
        if (!r.json || !r.json.success) {
          self._modalMsg((r.json && r.json.error) || L.errNetwork, true);
          m.sendBtn.disabled = false;
          return;
        }
        self._modalMsg(r.json.message || L.authCodeSent, false);
        m.code.focus();
        var left = 60;
        m.sendBtn.textContent = left + ' ' + L.authResendIn;
        self.codeTimer = setInterval(function () {
          left -= 1;
          if (left <= 0 || self.modal !== m) {
            clearInterval(self.codeTimer);
            self.codeTimer = null;
            if (self.modal === m) {
              m.sendBtn.disabled = false;
              m.sendBtn.textContent = L.authSendCode;
            }
            return;
          }
          m.sendBtn.textContent = left + ' ' + L.authResendIn;
      ...
      .catch(function () {
        if (self.destroyed || self.modal !== m) return;
        self._modalMsg(L.errNetwork, true);
        m.sendBtn.disabled = false;
      });
  };

  CommentWidget.prototype._verifyCode = function () {
    var self = this;
    var L = this.labels;
    var m = this.modal;
    if (!m || m.busy) return;
    ...
    if (!CODE_RE.test(code)) {
      this._modalMsg(L.errCodeInvalid, true);
      m.code.focus();
      return;
    }
    if (nick.length > MAX_NICK) {
      this._modalMsg(L.errNickTooLong, true);
      return;
    }
    m.busy = true;
    m.ok.disabled = true;
    this._authFetch('/verify', 'POST', { phone: phone, code: code, nickname: nick })
      .then(function (r) {
        if (self.destroyed || self.modal !== m) return;
        var json = r.json;
        if (!json || !json.success) {
          if (json && json.needs_nickname) m.nick.focus();
          self._modalMsg((json && json.error) || L.errNetwork, true);
          return;
        }
        if (nick) lsSet('nick', nick);
        self._setAuth(json.token, json.user);
        var pending = self.pendingSubmit;
        self.pendingSubmit = null;
        self._closeAuthModal();
        if (pending) self._submit(pending.form, pending.parentId, pending.onSuccess);
      })
      .catch(function () {
        if (self.destroyed || self.modal !== m) return;
        self._modalMsg(L.errNetwork, true);
